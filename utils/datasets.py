...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, crop_params=None):
    ...
    def __next__(self):
        ...
        if self.cp is not None:
            h, w, _ = img0.shape
            img0 = img0[
                int((self.cp[1] * h) / 100):int((self.cp[3] * h) / 100),
                int((self.cp[0] * w) / 100):int((self.cp[2] * w) / 100)
            ]
...
class LoadWebcam:  # for inference
    ...
    def __init__(self, pipe='0', img_size=640, stride=32, crop_params=None):
        self.img_size = img_size
        self.stride = stride
